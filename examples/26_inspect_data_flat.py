...
with h5py.File(DATASET_PATH, "r") as f:
    primitives = [p.decode("utf-8") if isinstance(p, bytes) else p for p in f.attrs["primitives"]]
    ...
    for scene_key in f.keys():
        ...
        poses = dp["poses"][()]  # (num_obj, 7)
        sizes = dp["sizes"][()]  # (num_obj, 3)
        feasibles = dp["feasibles"][()]  # (num_obj, num_primitives)
        target_poses = dp["target_poses"][()]  # (num_obj, num_primitives, 7)
        final_poses = dp["final_poses"][()]  # (num_obj, num_primitives, 7)

        num_objects = poses.shape[0]
        for oi in range(num_objects):
            volume = np.prod(sizes[oi])
            for pi, prim_name in enumerate(primitives):
                records.append(
                    {
                        "scene_id": scene_id,
                        "object_id": oi,
                        ...
                        "primitive_type": prim_name.split("_")[0],
                        "feasible": bool(feasibles[oi, pi]),
                        "target_pose": target_poses[oi, pi],
                        "final_pose": final_poses[oi, pi],
                    }
                )
...
df_pivot = df.pivot_table(index=["scene_id", "object_id"], columns="primitive_name", values="feasible").astype(int)
