...
class OffsetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return dp["depth"][()], dp["mask"][()], dp["target_offset"][()], dp["quat"][()], dp["feasibles"][()]


# %%
dataset = OffsetDataset()
...
def debug_model():
    ...
    depth, mask, target_offset, quat, _ = move_to_device(batch)

    vision_input = torch.stack([depth, mask], dim=1)
    ...
    logits = model(depth, mask, target_offset, quat)

    depth_impact = (logits - model(torch.zeros_like(depth), mask, target_offset, quat)).abs().mean()
    mask_impact = (logits - model(depth, torch.zeros_like(mask), target_offset, quat)).abs().mean()
    dist_impact = (logits - model(depth, mask, torch.zeros_like(target_offset), quat)).abs().mean()
    quat_impact = (logits - model(depth, mask, target_offset, torch.zeros_like(quat))).abs().mean()
...
def train():
    ...
    for batch in train_loader:
        depth, mask, target_offset, quat, y = move_to_device(batch)
...
def val():
    ...
    for batch in val_loader:
        depth, mask, target_offset, quat, y = move_to_device(batch)
        pred = model(depth, mask, target_offset, quat)
...
for epoch in range(100):
    ...
    if avg_f1 > best_f1:
        ...
        if patience_counter >= patience:
            ...
            torch.save(checkpoint, f"offset-model-{epoch}.pt")
            patience_counter = 0
