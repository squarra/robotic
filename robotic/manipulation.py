...
from robotic.scenario import Scenario
...
class Manipulation:
    ...
    def _grasp_obj(self, dim: int, align: tuple):
        ...
        self.komo.addObjective([1.0], FS.positionRel, [gripper, self.obj], OT.eq, scale=x_axis * 1e1)
        self.komo.addObjective([1.0], FS.positionRel, [gripper, self.obj], OT.ineq, scale=yz_plane * 1e1, target=[target])
        self.komo.addObjective([1.0], FS.positionRel, [gripper, self.obj], OT.ineq, scale=yz_plane * (-1e1), target=[-target])
        # gripper orientation
        self.komo.addObjective([0.8, 1.0], align[0], [gripper, self.obj], OT.eq, [1e0])
        self.komo.addObjective([0.8, 1.0], align[1], [gripper, self.obj], OT.eq, [1e0])
    ...
    def target_ori(self, ori: np.typing.ArrayLike):
